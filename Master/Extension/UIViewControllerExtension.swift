...
extension UIViewController {
    func shareWithComment(_ comment: String) {
          let textShare = [ comment ]
...
extension UIViewController {
    ...
    func backBarButtonAttribute(color: UIColor? = .appBackButtonColor, name: String? = "") {
        let backButton = UIBarButtonItem(title: name, style: UIBarButtonItem.Style.plain, target: self, action: nil)
        backButton.setTitleTextAttributes([NSAttributedString.Key.font: UIFont.appFont(with: 15)], for: .normal)
...
//        let vc =
//        self.addChild(vc)
    //  @objc       vc.view.frame = self.view.frame
...
extension UIViewController {
    ...
    func presentIOSAlertWarning(message: String, completion: @escaping () -> Void) {
        let title = NSAttributedString(string: "توجه", attributes: [NSAttributedString.Key.font : UIFont.appFont(with: 15), NSAttributedString.Key.foregroundColor : #colorLiteral(red: 0.08911790699, green: 0.08914073557, blue: 0.08911494166, alpha: 1)])
        let attributeMsg = NSAttributedString(string: message, attributes: [NSAttributedString.Key.font : UIFont.appFont(with: 13), NSAttributedString.Key.foregroundColor : UIColor.darkGray])
    ...
    func presentCDAlertWarningAlert(message: String, completion: @escaping () -> Void) {
        ...
        alert.titleFont = UIFont.appFont(with: 15)!
        alert.messageFont = UIFont.appFont(with: 13)!
        let done = CDAlertViewAction.init(title: "باشه", font: UIFont.appFont(with: 15), textColor: UIColor.darkGray, backgroundColor: .white) { (alert) -> Bool in
            completion()
            return true
        }
    ...
    func presentCDAlertWarningWithTwoAction(message: String, buttonOneTitle: String, buttonTwoTitle: String, handlerButtonOne: @escaping () -> Void, handlerButtonTwo: @escaping () -> Void) {
        ...
        alert.titleFont = UIFont.appFont(with: 15)!
        alert.messageFont = UIFont.appFont(with: 13)!
        let buttonOne = CDAlertViewAction.init(title: buttonOneTitle, font: UIFont.appFont(with: 15), textColor: UIColor.darkGray, backgroundColor: .white) { (alert) -> Bool in
            handlerButtonOne()
            return true
        }
        let buttonTwo = CDAlertViewAction.init(title: buttonTwoTitle, font: UIFont.appFont(with: 15), textColor: UIColor.darkGray, backgroundColor: .white) { (alert) -> Bool in
            handlerButtonTwo()
            return true
        }
